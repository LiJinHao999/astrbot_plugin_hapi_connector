...
# 托管操作记录相关键：改动后需要尝试 flush 补发（§2.3 防漏发）
...
async def save_plugin_config(plugin, patch: dict) -> dict:
    ...
    # 托管操作记录（§2.3 防漏发）：关 silent / 关 auto_approve / 改 mode/push/time
    ...
    if any(k in SUMMARY_KEYS for k in cleaned):
        ...
        if summary_svc is not None:
            try:
                ...
                logger.warning("config save 后操作记录 flush 失败: %s", e)
