...
2. **新增 `/hapi send <内容>` 指令，`cl`/`继续` 默认改发当前会话**  
   - `send` 发送到当前窗口选中的 session（`to` 需要序号，不适合做固定关键词）
   - 默认关键词 `cl`、`继续` 由「发给 1 号 session」改为「发给当前会话」，session 列表变动不再发错对象
   - 已保存旧默认映射（`to 1 /clear`、`to 1 继续`）的用户，插件启动时自动迁移为 `send` 版本；自定义的其它 `to` 映射不受影响

3. **新增会话创建模板**  
   - WebUI「交互优化 → 会话模板」把常用组合（代理 / 目录 / 机器 / 类型 / YOLO / 思考深度）存成命名模板
   - 聊天里 `/hapi create <模板名> [目录]` 一步创建，跳过向导；目录参数可覆盖模板默认，模板目录留空时必须传参
