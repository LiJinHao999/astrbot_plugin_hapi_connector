...
1. 支持多窗口（多会话）推送机制，现在可以借助群聊、私聊、不同管理员账户的对话窗口区分通知消息
在某个窗口使用sw/create命令后，将对话 session 的通知与管理绑定到当前会话

使用 hapi bind 命令设置主要推送窗口（有通知时默认发往此窗口，其它通知根据绑定逻辑路由）
使用 hapi bind reset 命令清除所有绑定路由关系 

不同对话窗口之间的通知、权限审批互相独立

简单说：你可以把bot拉进一个群聊，群聊内管理的远程 coding session 的通知推送与审批与你和 bot 的私聊相互独立，以便你区分多用户和任务

2. 支持了 hapi upload 命令，现在可以上传文件了。使用快捷发送时也可以直接在消息中附上图片。

2. 修复 Codex SSE 完成态判定，修复部分情况会出现的codex延迟通知问题
