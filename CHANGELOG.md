...
1. **真正修好 Focus / 快捷前缀发图重复**  
   上一版按内容 SHA-256 去重仍会漏：AstrBot 常把同一张图同时落成 `media_image_*.jpg` 与 `download.png` 两份本地缓存——路径不同、编码也不同（JPEG/PNG），字节哈希对不上。现提取附件时识别这种双缓存，只保留体积更大的那一份；`/hapi upload` 同一提取入口一并受益。内容哈希仍保留，用于挡掉路径不同但字节完全相同的副本。
...
2. **修复 Focus / 快捷前缀发送图片时同一张图被上传两次**（v3.2.2 补强）  
   AstrBot 常把同一附件同时落成 `media_image_*.jpg` 与 `download.jpg` 两份本地缓存，旧逻辑只按 path/url 去重，会把一份图当成两个 attachments 发出。v3.2.1 增加内容 SHA-256 去重；双缓存重编码场景见 v3.2.2。
